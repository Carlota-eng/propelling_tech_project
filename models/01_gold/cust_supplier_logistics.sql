...
supplier_top_parts AS (
    SELECT
        FK_SUPPLIER,
        MAX(CASE WHEN rn = 1 THEN AT_PART_NAME END) AS AT_TOP_SUPPLIED_PART_1,
        MAX(CASE WHEN rn = 2 THEN AT_PART_NAME END) AS AT_TOP_SUPPLIED_PART_2,
        MAX(CASE WHEN rn = 3 THEN AT_PART_NAME END) AS AT_TOP_SUPPLIED_PART_3
    FROM (
        SELECT 
            FK_SUPPLIER,
            AT_PART_NAME,
...
SELECT
    ...
    M.AT_REGION_NAME,
    M.AT_NATION_NAME,
    P.AT_TOP_SUPPLIED_PART_1,
    P.AT_TOP_SUPPLIED_PART_2,
    P.AT_TOP_SUPPLIED_PART_3,
